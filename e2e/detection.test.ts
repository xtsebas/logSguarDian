...
describe("E2E Detection Suite (PLAN.md F5.7 GATE)", () => {
  ...
  test("benign multi-field post is not blocked (no threshold stopgap needed)", async () => {
    ...
    // (fixed by non_form_operator_count) together caused this to score sqli.
    // Resolved at the feature level — the model predicts benign outright, so
    // no per-class or elevated threshold is needed at the single global 0.35.
    const res = await request(app)
      .post("/posts")
      .type("form")
